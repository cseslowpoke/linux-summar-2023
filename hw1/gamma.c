...
#define verify(x)                                                              \
  do {                                                                         \
    int e;                                                                     \
    if ((e = x) != 0) {                                                        \
      fprintf(stderr, "%s(%d) %s: %s\n", __FILE__, __LINE__, #x, strerror(e)); \
      exit(1);                                                                 \
    }                                                                          \
  } while (0)
...
void qsort_mt(void *a, size_t n, size_t es, cmp_t *cmp, int maxthreads,
              int forkelem) {
  struct qsort *qs;
  struct common c;
  int i, islot;
  ...
  c.swaptype = ((char *)a - (char *)0) % sizeof(long) || es % sizeof(long) ? 2
               : es == sizeof(long)                                        ? 0
                                                                           : 1;
  c.es = es;
  c.cmp = cmp;
...
again:
  ...
  if (c->idlethreads == c->nthreads) {
    for (i = 0; i < c->nthreads; i++) {
      ...
      qs2->st = ts_term;
      verify(pthread_cond_wait(&qs2->cond_st, &qs2->mtx_st)); // JJJJ
      verify(pthread_mutex_unlock(&qs2->mtx_st));
    }
    verify(pthread_mutex_unlock(&c->mtx_al));
    return NULL;
